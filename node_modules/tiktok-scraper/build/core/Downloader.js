...
class Downloader {
    constructor({ progress, proxy, test, noWaterMark, userAgent, filepath, bulk }) {
    ...
    zipIt({ collector, filepath, fileName, asyncDownload }) {
        return new Promise((resolve, reject) => {
            const zip = filepath ? `${filepath}/${fileName}.zip` : `${fileName}.zip`;
            const output = fs_1.createWriteStream(zip);
